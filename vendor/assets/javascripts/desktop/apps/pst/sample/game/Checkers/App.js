define([
	 ...
	 "dojo/i18n!./nls/app"	 
],function(array, declare, lang, dojo,i18n,_FormValueWidget,Button,TextBox,Toolbar,gfx,Moveable,_App,Window,StatusBar,nlsApp) {

return declare([_App], {
	init: function(args){
		var app = nlsApp;//dojo.i18n.getLocalization("scene", "apps",this.lang);
		this.win = new Window({
        	app  : this,
			title: app.title,
			width: "400px",
			...
			resizable: false,
            showMaximize: false,
			iconClass: "icon-16-apps-preferences-scene-sound",
			onClose: dojo.hitch(this, this.kill)
		});
		this.win.show();
		this.surface = gfx.createSurface(this.win.containerNode, 400, 400);
		this.drawBoard();
		this.win.startup();
	},
	kill: function(){
	...
	makePiece: function(c)
	{
		...
		var move = new gfx.Moveable(shape);
