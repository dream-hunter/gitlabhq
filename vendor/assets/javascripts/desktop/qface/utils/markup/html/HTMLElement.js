define([
	...
	"qface/utils/markup/html/HTMLComment",
	"qface/utils/markup/html/HTMLAttribute"
], function(declare, HTMLModel,HTMLItem, HTMLText, HTMLComment, HTMLAttribute) {
...
return declare(HTMLItem, {
	...
	_formatModel: function( newElement, index, context) {
		...
		function formatElem(elem, context) {
			...
			if (s) {
				...
				var doFormat;
				if (!HTMLModel._noFormatElements[elem.tag]) {
					context.indent += 2;
		...
		if (!HTMLModel._noFormatElements[newElement.tag]) {
	...
	removeAttribute: function(name) {
		this.attributes.every(function(attr, idx, arr) {
			if (attr.name === name) {
				...
				// see code in widget.createWidget().
	...
	setText: function (text) {
		...
		var result = require("qface/utils/markup/html/HTMLParser").parse(text,this);
