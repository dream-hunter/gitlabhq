define([
	...
	"qface/utils/markup/html/HTMLParser",
	"qface/utils/markup/css/CSSSelector",
...
return declare(HTMLItem, {
	...
	save: function (isWorkingCopy) {
		var deferred;
		var file = require("qface/services/ofs/FileSystem").findSync(this.fileName);
		if (file) {
			var text = this.getText();
			deferred = file.setContent(text,isWorkingCopy);
		}
		return deferred;
	},
	...
	addStyleSheet: function(url, content, dontLoad, beforeChild, loader) {
		...
		if (!dontLoad) {
			...
			this._loadedCSS[url] = require("qfacex/ui/ide/model/Factory").getModel({
				url : url,
				includeImports : true,
				loader : loader
			});
	...
	close: function() {
		...
		require("qfacex/ui/ide/model/Factory").closeModel(this);
	},
