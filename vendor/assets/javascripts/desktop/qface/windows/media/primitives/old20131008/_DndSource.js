...
// summary:
//		Handles drag and drop operations (as a source or a target) for Canvas`
...
var dndSource = declare(_dndSelector, {
	// summary:
	//		Handles drag and drop operations (as a source or a target) for `Canvas`
	...
	constructor: function(/*qface/windows/media/_host/Canvas*/ canvas, params){
		...
		var type = params.accept instanceof Array ? params.accept : ["text", "canvas"];
		this.accept = null;
		if(type.length){
			this.accept = {};
	...
	checkAcceptance: function(/*===== source, nodes =====*/){
		...
		// source: qface/windows/media/_html/dndSource
	...
	_onDragMouse: function(e, firstTime){
		...
		if(newTarget && this.betweenThreshold > 0){
			// If mouse is over a new Visual, then get new Visual's position and size
			if(!this.targetBox || oldTarget != newTarget){
				this.targetBox = canvas.position(newTarget);
			}
		...
		if(firstTime || newTarget != oldTarget || newDropPosition != oldDropPosition){
			...
			if(!newTarget){
				m.canDrop(false);
			}else if(newTarget == canvas.rootVisual && newDropPosition != "Over"){
	...
	onMouseMove: function(e){
		...
		if(this.isDragging){
			...
			if(this.mouseDown && this.isSource &&
				...
				var nodes = this.getSelectedVisuals();
				if(nodes.length){
					if(nodes.length > 1){
	...
	_isParentChildDrop: function(source, targetVisual){
		// summary:
		//		Checks whether the dragged items are parent visuals in the canvas which are being
		...
		//
		// targetVisual:
		//		The visual onto which the dragged nodes are being dropped.
		//
		// tags:
		//		private

		// If the dragged object is not coming from the canvas this widget belongs to,
		// it cannot be invalid.
		if(!source.canvas || source.canvas != this.canvas){
			return false;
		}


		var rootVisual = source.canvas.rootVisual;
		var ids = source.selection;

		var visual = targetVisual.parent;

		// Iterate up the visual hierarchy from the target drop visual,
		// checking of any of the dragged nodes have the same ID.
		while(visual != rootVisual && !ids[visual.id]){
			visual = visual.parent;
		}

		return visual.id && ids[visual.id];
	},
