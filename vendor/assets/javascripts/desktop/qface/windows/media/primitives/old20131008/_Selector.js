define( [
	...
	"dojo/touch",
	"./_Container"
], function(array, connect, declare, Deferred, kernel, lang, cookie, mouse, on, touch, _Container){

	// module:
	//		qface/windows/media/_dom/_dndSelector


	return declare(_Container, {
		// summary:
		//		This is a base class for `qface/windows/media/_base/dndSource` , and isn't meant to be used directly.
		...
		addVisual: function( visual, /*Boolean?*/isAnchor){
			// summary:
			//		add visual to current selection
			// visual: Visual
			//		visual to add
			// isAnchor: Boolean
			//		Whether the visual should become anchor.

			this.setSelection(this.getSelectedVisuals().concat( [node] ));
			if(isAnchor){ this.anchor = node; }
			return visual;
		},
		removeVisual: function( visual){
			// summary:
			//		remove visual from current selection
			// visual: Visual
			//		visual to remove
			this.setSelection(this._setDifference(this.getSelectedVisuals(), [node]));
			return visual;
		},
		isVisualSelected: function( visual){
			// summary:
			//		return true if visual is currently selected
			// visual: Visual
			//		the visual to check whether it's in the current selection

			return visual.id && !!this.selection[visual.id];
		},
		setSelection: function(newSelection){
			...
			var oldSelection = this.getSelectedVisuals();
			array.forEach(this._setDifference(oldSelection, newSelection), lang.hitch(this, function(visual){
				visual.setSelected(false);
				if(this.anchor == visual){
					delete this.anchor;
				}
				delete this.selection[visual.id];
			}));
			array.forEach(this._setDifference(newSelection, oldSelection), lang.hitch(this, function(visual){
				visual.setSelected(true);
				this.selection[visual.id] = node;
			}));
		...
		_updateSelectionProperties: function(){
			...
			var selected = this.getSelectedVisuals();
			var paths = [], visuals = [], selects = [];
