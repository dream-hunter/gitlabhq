...
define( [
	...
	var Command = declare(null,{
		/**
		* Inner class used by the {{#crossLink "DrawContext"}}{{/crossLink}} class. Used to create the instruction lists used in DrawContext:
		* @class Command
		* @protected
		* @constructor
		**/
	...
	var DrawingContext = declare(null,{
		"-privates-" : {
			/**
		...
		// private methods:
			...
			_updateInstructions : function() {
				...
				this._instructions.push(DrawingContext.beginCmd);
				...
				if (this._strokeInstructions) {
					...
					if (this._ignoreScaleStroke) {
						this._instructions.push(
							...
							DrawingContext.strokeCmd,
							new Command(this._ctx.restore, [], false)
						);
					} else {
						this._instructions.push(DrawingContext.strokeCmd);
					}
				}
			},
		...
		"-methods-" : {
			/**
			 * Returns true if this DrawingContext instance has no drawing commands.
			 * @method isEmpty
			 * @return {Boolean} Returns true if this DrawingContext instance has no drawing commands.
			 **/
			isEmpty : function() {
			...
			/**
			 * Draws only the path described for this DrawingContext instance, skipping any non-path instructions, including fill and
		...
		// public methods that map directly to context 2D calls:
			 ...
			 * @return {DrawingContext} The DrawingContext instance the method is called on (useful for chaining calls.)
			 **/
			moveTo : function(x, y) {
		...
		// public methods that roughly map to Flash graphics APIs:
			/**
			 * Clears all drawing instructions, effectively resetting this DrawingContext instance.
			 * @method clear
			 * @return {DrawingContext} The DrawingContext instance the method is called on (useful for chaining calls.)
			 **/
			clear : function() {
			...
			beginFill : function(color) {
				...
				this._fillInstructions = color ? [new Command(this._setProp, ["fillStyle", color], false), DrawingContext.fillCmd] : null;
				return this;
			},
			 ...
			 *
			 *      myDrawContext.beginLinearGradientFill(["#000","#FFF"], [0, 1], 0, 20, 0, 120).drawRect(20, 20, 120, 120);
			 *
			...
			beginLinearGradientFill : function(colors, ratios, x0, y0, x1, y1) {
				...
				this._fillInstructions = [new Command(this._setProp, ["fillStyle", o], false), DrawingContext.fillCmd];
				return this;
			},
			 ...
			 *
			 *      myDrawContext.beginRadialGradientFill(["#F00","#00F"], [0, 1], 100, 100, 0, 100, 100, 50).drawCircle(100, 100, 50);
			 *
			...
			beginBitmapFill : function(image, repetition, matrix) {
				...
				if (matrix) {
					arr = [
						...
						DrawingContext.fillCmd,
						new Command(this._ctx.restore, [], false)
					];
				} else {
					arr = [cmd, DrawingContext.fillCmd];
				}
			 ...
			 *
			 *      myDrawContext.setStrokeStyle(8,"round").beginStroke("#F00");
			 *
			...
			setStrokeStyle : function(thickness, caps, joints, miterLimit, ignoreScale) {
				...
				this._strokeStyleInstructions = [
					...
					new Command(this._setProp, ["lineCap", (caps == null ? "butt" : (isNaN(caps) ? caps : DrawingContext.STROKE_CAPS_MAP[caps]))], false),
					new Command(this._setProp, ["lineJoin", (joints == null ? "miter" : (isNaN(joints) ? joints : DrawingContext.STROKE_JOINTS_MAP[joints]))], false),
			 ...
			 *
			 *      myDrawContext.setStrokeStyle(10).beginLinearGradientStroke(["#000","#FFF"], [0, 1], 0, 20, 0, 120).drawRect(20, 20, 120, 120);
			 *
			 ...
			 *
			 *      myDrawContext.setStrokeStyle(10)
			 ...
			 *
			 *      var g = new DrawingContext();
			 *	    g.setStrokeStyle(1);
			 *	    g.beginStroke(DrawingContext.getRGB(0,0,0));
			 *	    g.beginFill(DrawingContext.getRGB(255,0,0));
			 *	    g.drawCircle(0,0,3);
			 *
			 *	    var s = new Shape(g);
			 *		s.x = 100;
			...
			/**
			 * Draws an ellipse (oval) with a specified width (w) and height (h). Similar to {{#crossLink "DrawingContext/drawCircle"}}{{/crossLink}},
			...
			decodePath : function(str) {
				...
				var params = [];
				var x=0, y=0;
				var base64 = DrawingContext.BASE_64;
			...
			/**
			 * Returns a clone of this DrawingContext instance.
			 * @method clone
			 * @return {DrawingContext} A clone of the current DrawingContext instance.
			 **/
			clone : function() {
				var o = new DrawingContext();
		...
		/**
		 * The DrawingContext class exposes an easy to use API for generating vector drawing instructions and drawing them to a
		 * specified context. Note that you can use DrawingContext without any dependency on the Easel framework by calling {{#crossLink "DisplayObject/draw"}}{{/crossLink}}
		 ...
		 *	    stage.update();
		 *
		 * Note that all drawing methods in DrawingContext return the DrawingContext instance, so they can be chained together. For example,
		 ...
		 *
		 *      myDrawContext.beginStroke("#F00").beginFill("#00F").drawRect(20, 20, 100, 50).draw(myContext2D);
		 *
		 * <h4>Tiny API</h4>
		 * The DrawingContext class also includes a "tiny API", which is one or two-letter methods that are shortcuts for all of the
		 * DrawingContext methods. These methods are great for creating compact instructions, and is used by the Toolkit for CreateJS
		 * to generate readable code. All tiny methods are marked as protected, so you can view them by enabling protected
		 * descriptions in the docs.
		 *
		 * <table>
		 *     <tr><td><b>Tiny</b></td><td><b>Method</b></td><td><b>Tiny</b></td><td><b>Method</b></td></tr>
		 *     <tr><td>mt</td><td>{{#crossLink "DrawingContext/moveTo"}}{{/crossLink}} </td>
		 *     <td>lt</td> <td>{{#crossLink "DrawingContext/lineTo"}}{{/crossLink}}</td></tr>
		 *     <tr><td>at</td><td>{{#crossLink "DrawingContext/arcTo"}}{{/crossLink}} </td>
		 *     <td>bt</td><td>{{#crossLink "DrawingContext/bezierCurveTo"}}{{/crossLink}} </td></tr>
		 *     <tr><td>qt</td><td>{{#crossLink "DrawingContext/quadraticCurveTo"}}{{/crossLink}} (also curveTo)</td>
		 *     <td>r</td><td>{{#crossLink "DrawingContext/rect"}}{{/crossLink}} </td></tr>
		 *     <tr><td>cp</td><td>{{#crossLink "DrawingContext/closePath"}}{{/crossLink}} </td>
		 *     <td>c</td><td>{{#crossLink "DrawingContext/clear"}}{{/crossLink}} </td></tr>
		 *     <tr><td>f</td><td>{{#crossLink "DrawingContext/beginFill"}}{{/crossLink}} </td>
		 *     <td>lf</td><td>{{#crossLink "DrawingContext/beginLinearGradientFill"}}{{/crossLink}} </td></tr>
		 *     <tr><td>rf</td><td>{{#crossLink "DrawingContext/beginRadialGradientFill"}}{{/crossLink}} </td>
		 *     <td>bf</td><td>{{#crossLink "DrawingContext/beginBitmapFill"}}{{/crossLink}} </td></tr>
		 *     <tr><td>ef</td><td>{{#crossLink "DrawingContext/endFill"}}{{/crossLink}} </td>
		 *     <td>ss</td><td>{{#crossLink "DrawingContext/setStrokeStyle"}}{{/crossLink}} </td></tr>
		 *     <tr><td>s</td><td>{{#crossLink "DrawingContext/beginStroke"}}{{/crossLink}} </td>
		 *     <td>ls</td><td>{{#crossLink "DrawingContext/beginLinearGradientStroke"}}{{/crossLink}} </td></tr>
		 *     <tr><td>rs</td><td>{{#crossLink "DrawingContext/beginRadialGradientStroke"}}{{/crossLink}} </td>
		 *     <td>bs</td><td>{{#crossLink "DrawingContext/beginBitmapStroke"}}{{/crossLink}} </td></tr>
		 *     <tr><td>es</td><td>{{#crossLink "DrawingContext/endStroke"}}{{/crossLink}} </td>
		 *     <td>dr</td><td>{{#crossLink "DrawingContext/drawRect"}}{{/crossLink}} </td></tr>
		 *     <tr><td>rr</td><td>{{#crossLink "DrawingContext/drawRoundRect"}}{{/crossLink}} </td>
		 *     <td>rc</td><td>{{#crossLink "DrawingContext/drawRoundRectComplex"}}{{/crossLink}} </td></tr>
		 *     <tr><td>dc</td><td>{{#crossLink "DrawingContext/drawCircle"}}{{/crossLink}} </td>
		 *     <td>de</td><td>{{#crossLink "DrawingContext/drawEllipse"}}{{/crossLink}} </td></tr>
		 *     <tr><td>dp</td><td>{{#crossLink "DrawingContext/drawPolyStar"}}{{/crossLink}} </td>
		 *     <td>p</td><td>{{#crossLink "DrawingContext/decodePath"}}{{/crossLink}} </td></tr>
		 * </table>
		 *
		 * Here is the above example, using the tiny API instead.
		 *
		 *      myDrawContext.s("#F00").f("#00F").r(20, 20, 100, 50).draw(myContext2D);
		 *
	...
	var p = DrawingContext.prototype;
	/**
	 * Maps the familiar ActionScript <code>curveTo()</code> method to the functionally similar {{#crossLink "DrawingContext/quadraticCurveTo"}}{{/crossLink}}
	 * method.
	 * @method curveTo
	 * @type {Function}
	 **/
	...
	/**
	 * Maps the familiar ActionScript <code>drawRect()</code> method to the functionally similar {{#crossLink "DrawingContext/rect"}}{{/crossLink}}
	 * method.
	 * @method drawRect
	 * @type {Function}
	 **/
...
// static public methods:
	 ...
	 *
	 *      DrawingContext.getRGB(50, 100, 150, 0.5);
	 *
	 ...
	 *
	 *      DrawingContext.getRGB(0xFF00FF, 0.2);
	 *
	 ...
	 **/
	DrawingContext.getRGB = function(r, g, b, alpha) {
		if (r != null && b == null) {
			alpha = g;
			b = r&0xFF;
			g = r>>8&0xFF;
	 ...
	 *
	 *      DrawingContext.getHSL(150, 100, 70);
	 *
	 * @method getHSL
	 * @static
	 ...
	 **/
	DrawingContext.getHSL = function(hue, saturation, lightness, alpha) {
		if (alpha == null) {
	...
	/**
	 * Map of Base64 characters to values. Used by {{#crossLink "DrawingContext/decodePath"}}{{/crossLink}}.
	 * @property BASE_64
	 * @static
	 * @final
	 * @type {Object}
	 **/
	DrawingContext.BASE_64 = {"A":0,"B":1,"C":2,"D":3,"E":4,"F":5,"G":6,"H":7,"I":8,"J":9,"K":10,"L":11,"M":12,"N":13,"O":14,"P":15,"Q":16,"R":17,"S":18,"T":19,"U":20,"V":21,"W":22,"X":23,"Y":24,"Z":25,"a":26,"b":27,"c":28,"d":29,"e":30,"f":31,"g":32,"h":33,"i":34,"j":35,"k":36,"l":37,"m":38,"n":39,"o":40,"p":41,"q":42,"r":43,"s":44,"t":45,"u":46,"v":47,"w":48,"x":49,"y":50,"z":51,"0":52,"1":53,"2":54,"3":55,"4":56,"5":57,"6":58,"7":59,"8":60,"9":61,"+":62,"/":63};
		
	
	/**
	 * Maps numeric values for the caps parameter of {{#crossLink "DrawingContext/setStrokeStyle"}}{{/crossLink}} to
	 * corresponding string values. This is primarily for use with the tiny API. The mappings are as follows: 0 to
	 * "butt", 1 to "round", and 2 to "square".
	 * For example, to set the line caps to "square":
	 *
	 *      myDrawContext.ss(16, 2);
	 *
	 ...
	 * @type {Array}
	 **/
	DrawingContext.STROKE_CAPS_MAP = ["butt", "round", "square"];
	
	/**
	 * Maps numeric values for the joints parameter of {{#crossLink "DrawingContext/setStrokeStyle"}}{{/crossLink}} to
	 * corresponding string values. This is primarily for use with the tiny API. The mappings are as follows: 0 to
	 * "miter", 1 to "round", and 2 to "bevel".
	 * For example, to set the line joints to "bevel":
	 *      myDrawContext.ss(16, 0, 2);
	 *
	 ...
	 * @type {Array}
	 **/
	DrawingContext.STROKE_JOINTS_MAP = ["miter", "round", "bevel"];
	 ...
	 **/
	DrawingContext._ctx = (createjs.createCanvas?createjs.createCanvas():document.createElement("canvas")).getContext("2d");
	 ...
	 * @type {Command}
	 **/
	DrawingContext.beginCmd = new Command(DrawingContext._ctx.beginPath, [], false);
	 ...
	 * @type {Command}
	 **/
	DrawingContext.fillCmd = new Command(DrawingContext._ctx.fill, [], false);
	 ...
	 * @type {Command}
	 **/
	DrawingContext.strokeCmd = new Command(DrawingContext._ctx.stroke, [], false);
