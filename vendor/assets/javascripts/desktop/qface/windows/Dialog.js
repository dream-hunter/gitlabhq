...
DialogClass.showDialog = function(title, content, style, callback, okLabel, hideCancel, submitOnEnter) {
	...
	var submitButton = new Button({label: okLabel ? okLabel : nlsCommon.ok, type: "submit", "class": "maqPrimaryButton"})
	...
	if (!hideCancel) {
		...
		dialogActions.appendChild(new Button({label: nlsCommon.cancel, onClick: _onCancelClick, "class": "maqSecondaryButton"}).domNode);
	}
