  ...
  "dijit/registry",
...
],function(lang,declare,array,connect,fx,coreFx,domStyle,domClass,dom,topic,registry,utilHtml,Applet) {
    ...
    config: {fx:2,window:{animSpeed:275,constrain:false}},
      ...
      if(store.getValue(item, "id").indexOf("load") == -1)
				this._winconnects[store.getValue(item, "id")] = connect.connect(domNode, "onclick", registry.byId(store.getValue(item, "id")), "_onTaskClick");
			...
			if(this.config.fx > 0){
				...
				fx.fadeIn({node: domNode, duration: this.config.window.animSpeed}).play();
			}
			if(store.getValue(item, "id").indexOf("load") == -1)
				registry.byId(store.getValue(item, "id"))._menu.bindDomNode(domNode);
		},
		
		onDelete: function(item){
			var node = this._buttons[item.id[0]];
			if(item.id[0].indexOf("load") == -1)
				connect.disconnect(this._winconnects[item.id[0]]);
			var onEnd = function(){
			...
			if (this.config.fx >= 1){
				...
				var anim = coreFx.chain([fade, slide]);
				connect.connect(slide, "onEnd", null, onEnd);
				anim.play();
			}
			else onEnd();
		},
		...
		setupLaunchApp: function(){
			this._onLaunch = topic.subscribe("/qface/system/desktop/scene/launchApp",
			lang.hitch(this, function(scene,sysname,name){
				...
				var onEnd = topic.subscribe("/qface/system/desktop/scene/launchAppEnd",function(){
