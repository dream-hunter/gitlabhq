...
var DesignOutlineTreeModel = declare(null, {
	...
	getLabel: function(item) {
		...
		var widget = Widget.byId(widget.id, this._context.getDocument(),this._context);
		if (widget) {
			var id = widget.getId();
			if (id ) {
				label += " id=" + id;
	...
	_getWidget: function(item) {
		return Widget.byId(item.id,this._context.getDocument(),this._context);
	},
		...
		connect.subscribe("/maqetta/appstates/state/changed/end", this,
			function(e) {
				var declaredClass = (typeof davinci !== "undefined") &&
						this._workbench.currentEditor &&
						this._workbench.currentEditor.declaredClass;
