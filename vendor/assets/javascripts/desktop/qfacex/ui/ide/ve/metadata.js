define("qfacex/ui/ide/ve/metadata", [
	"require",
	"dojo/Deferred",
    "dojo/promise/all",
    "dojo/_base/lang",
    "dojo/_base/connect",
   // "qfacex/ui/ide/Workbench",
	"qface/lang/Path",
	"../repositoryinfo"
], function(require, Deferred, all, lang, connect, Path, info) {
    var davinciLibPath = require.toUrl("qfacex/ui/ve/"); //modified by LWF

	var Metadata,
		Workbench,
		
	//moved from library.js by LWF
	 ...
	 library={_customWidgets:{}},
		_themesCache = {},
		_themesMetaCache = {},
		_userLibsCache = { //modified by LWF
			base : [
				{
					hasSource : true,
					id : "dojo",
					metaRoot : "metadata/dojo/1.8",
					required : false,
					root	 : "lib/dojo",
					version  : "1.8"
				},
				...
				},
				{
					hasSource : false,
					id : "maqetta",
					metaRoot : "metadata/maqetta/core/0.3",
					required : true,
					root	 : "lib/maqetta",
					version  : "0.3"
				}
	...
	function parsePackage(pkg, path) {
		...
		//path = new Path(path); //modified by LWF
        ...
        var packages = [ {
            ...
            location : pkg.directories.metadata?path+"/"+pkg.directories.metadata :path//locPath.append(path).append(pkg.directories.metadata).toString()  //modified by LWF
        } ];
        if (pkg.name != "dojo") {
            ...
            var libPath = davinciLibPath+'static/lib/' + pkg.name + '/' + pkg.version; // modified by LWF

            packages.push({
                name: pkg.__libraryModuleId,
                location: libPath //locPath.append(libPath).toString()   // modified by LWF
            });
        }
        require = require({
            packages: packages
		...
		if (lang.exists("scripts.widget_metadata", pkg)) {
			if (typeof pkg.scripts.widget_metadata == "string") {
				var widgetsJsonPath = path+"/"+pkg.scripts.widget_metadata;//var widgetsJsonPath = path.append(pkg.scripts.widget_metadata); //modified by LWF
				deferred = dojo.xhrGet({
					url : widgetsJsonPath + "?" + info.revision,
					handleAs : "json"
				}).then(function(data) {
					if (data) {
						return parseLibraryDescriptor(pkg.name, data,
								path )//widgetsJsonPath.getParentPath()); // lop off "*.json" // modified by LWF
		            }
		        });
			} else {
	...
	function parseLibraryDescriptor(libName, descriptor, path) {
        ...
        descriptor.$path =path;// path.toString();//modified by LWF
		...
		wm.widgets.forEach(function(item) {
        	...
        	if (item.icon && !item.iconLocal) {
                item.icon = path + "/"+item.icon; //path.append(item.icon).toString(); //modified by LWF
            }
...
//    };
	...
	Metadata = {
		...
		init: function() {
			...
			if (!_inited) {
				_inited = true;
				getUserLibs().forEach(function(lib) {
	...
	// (or a combined object).  Putting it here for now, to quickly integrate.
					var path = davinciLibPath+lib.metaRoot;//Library.getMetaRoot(lib.id, lib.version); //modified by LWF
					if (path) {
        ...
        getSmartInput: function(type) {
        	...
        	if (type in smartInputCache) {
	        	...
	        	if (!moduleId) {
	        		...
	        		require(["qfacex/ui/ide/ve/input/SmartInput"], function(SmartInput) {
