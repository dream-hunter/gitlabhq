define([
		...
		"qfacex/ui/workbench/ViewLite",
    	"qfacex/ui/ide/ve/widgets/HTMLStringUtil",
    	"qfacex/ui/ide/ve/widgets/WidgetToolBar",
    	"qfacex/ui/ide/ve/widgets/Cascade",
    	"qfacex/ui/ide/ve/widgets/CommonProperties",
    	"qfacex/ui/ide/ve/widgets/WidgetProperties",
    	"qfacex/ui/ide/ve/widgets/EventSelection"
], function(require,declare, veNls, commonNls, TabContainer, ContentPane,ViewLite, HTMLStringUtil,
		   	WidgetToolBar,  Cascade, CommonProperties, WidgetProperties, EventSelection
		    ){
return declare([ViewLite], {
	...
	constructor: function(params, srcNodeRef){
		...
		this.subscribe("/davinci/states/state/changed", this._stateChanged);
		this.subscribe("/maqetta/appstates/state/changed",  this._stateChanged);
    	this.subscribe("/davinci/ui/initialPerspectiveReady", this._initialPerspectiveReady);
    	this.subscribe("/davinci/workbench/ready",  this._workbenchReady);
	},
...
/*
	          {key: "common",
	        	  ...
	        	  pageTemplate:{html: "<div data-dojo-type='qfacex/ui/ide/ve/widgets/CommonProperties' data-dojo-props='workbench:this._workbench'></div>"}},
*/
	          // NOTE: the first section (widgetSpecific) is injected within buildRendering()
	          {key: "widgetSpecific",
	        	  className:'maqPropertySection page_editor_only',
	        	  addCommonPropertiesAtTop:false,
	        	  html: "<div data-dojo-type='qfacex/ui/ide/ve/widgets/WidgetProperties' data-dojo-props='workbench:this._workbench'></div>"},  
	        	  
	          // NOTE: other sections are added dynamically via first call to _beforeEditorSelected
	          {key: "events",
	        	  className:'maqPropertySection page_editor_only',
	        	  addCommonPropertiesAtTop:false,
		          pageTemplate:{html: "<div data-dojo-type='qfacex/ui/ide/ve/widgets/EventSelection' data-dojo-props='workbench:this._workbench'></div>"}},
	          {key: "layout",
	...
	buildRendering: function(){
		//this.domNode = dojo.doc.createElement("div"); // modified by LWF 20130426
		//dojo.addClass(this.domNode,'propertiesContent');
		var template="<div class='propertiesContent'>";
		template+=this._titleBarDiv;
		template+="<div class='propertiesToolBar' data-dojo-type='qfacex/ui/ide/ve/widgets/WidgetToolBar' data-dojo-props='workbench:this._workbench'></div>";
		template+="<div data-dojo-type='qfacex/ui/ide/ve/widgets/WidgetProperties' data-dojo-props='workbench:this._workbench'></div>";
	...
	startup : function(){
		...
		this._editor = this.getWorkbench().currentEditor;
		...
		this.subscribe("/davinci/ui/widgetValuesChanged", this._widgetValuesChanged);
		this.subscribe("/davinci/ui/widgetPropertiesChanged", this._widgetPropertiesChanged);
	...
	_editorSelected : function(editorChange){
		...
		if(this._editor){
			if (!this._editor.isInstanceOf(require("qfacex/ui/ide/ve/PageEditor"))){
				//Hide all sections
...
*/
		if(!this._alreadySplitIntoMultipleTabs){
			...
			for(var i=0;i<this.pageTemplate.length;i++){
				...
				topContent += "<div class='propertiesToolBar' data-dojo-type='qfacex/ui/ide/ve/widgets/WidgetToolBar' data-dojo-props='workbench:this._workbench'></div>";
				topContent += "<div class='cascadeBackButtonDiv'><button onclick='require(\"qfacex/ui/ide//ve/widgets/HTMLStringUtil\").showSection(\""+key+"\",\""+title+"\")'>"+title+" "+veNls.properties+"</button></div>";
				...
				if(i==0){
					...
					var cp = new ContentPane({title:title, content:content, 'class':className,_contentSetterParams:{propsThis:this}});
				...
				if(closeBoxNodes.length > 0){
					...
					dojo.connect(closeBox, 'click', this, function(event){
						this._workbench.collapsePaletteContainer(event.currentTarget);
					});
				}
			}
	...
	_updateToolBars: function(){
		...
		propertyToolbarContainers.forEach(function(container){
			if (this._editor && this._editor.isInstanceOf(require("qfacex/ui/ide/ve/PageEditor"))) {
