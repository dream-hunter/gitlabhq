define([
	...
	"qfacex/ui/ide/html/ui/HTMLEditor",
	"qface/lang/Path",
	"qfacex/ui/ide/ve/VisualEditor",
	"qfacex/ui/ide/ve/VisualEditorOutline",
	"qfacex/ui/ide/ve/widget",
	"qfacex/util/GeomUtils",
	"dojo/i18n!./nls/ve"
], function(require,declare, ModelEditor, BorderContainer, ContentPane, Moveable, CommandStack, HTMLEditor, Path, VisualEditor, VisualEditorOutline, widgetUtils, GeomUtils, veNls){
...
return declare( ModelEditor, {
    ...
    constructor: function (element, fileName,existWhenVisible,workbench) {
        ...
        var htmlEditor = this.htmlEditor = new HTMLEditor(this._srcCP.domNode, fileName, true,workbench);
        ...
        this.subscribe("/davinci/ui/styleValuesChange", dojo.hitch(this,this._stylePropertiesChange));
        this.subscribe("/davinci/ui/widgetSelected",   dojo.hitch(this._widgetSelectionChange));
        this.subscribe("/davinci/ui/selectionChanged",  dojo.hitch(this._modelSelectionChange));
//      this._connect(this.visualEditor.context, "onSelectionChange","_widgetSelectionChange");
		this.subscribe("/davinci/ui/editorSelected", dojo.hitch(this._editorSelected));
		this.subscribe("/davinci/ui/context/loaded", dojo.hitch(this._contextLoaded));
		this.subscribe("/davinci/ui/deviceChanged", dojo.hitch(this._deviceChanged));
    },
...
//			this.visualEditor.onContentChange();
	...
	_editorSelected: function(event){
		...
		if(event.editor && event.editor.editorContainer && 
				(event.editor.isInstanceOf(require("qfacex/ui/ide/ve/PageEditor")) ||
	...
	_contextLoaded: function(){
		if(this._workbench.currentEditor == this && this.editorContainer){
			this.editorContainer.updateToolbars();
		}
	},
	...
	_selectLayout: function(layout){
		...
		require(["qfacex/ui/ide/actions/SelectLayoutAction"], function(ActionClass){
	...
	_modelSelectionChange: function (selection) {
	     ...
	     */
		if( this._displayMode == "source" || this._workbench.currentEditor !== this ) {
			return;
		}
...
//		this._srcChanged();
	...
	_srcChanged: function() {
		...
		this._workbench.publish("/davinci/ui/context/pagerebuilt", context);
		if(statesScenes){
