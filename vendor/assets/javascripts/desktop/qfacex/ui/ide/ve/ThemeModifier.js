define( [
	...
	"qfacex/ui/ide/ve/commands/ModifyRuleCommand",
	"qfacex/ui/ide/ve/commands/StyleCommand",
	"dojo/i18n!qfacex/ui/ide/ve/nls/common"
], function(declare, Path, Factory, URLRewrite, ModifyRuleCommand, StyleCommand, commonNls) {
...
return declare( null, {
	...
	_getThemeResource: function (fileName) {
		...
		return workspace.findSync(absoluteLocation);
	},
				...
				visit: function(node){
					if( node.elementType=="CSSFile" && node.isDirty()){
						...
						deferred.then(function(){
							...
							if (!isAutoSave){
								this._workbench.getUserWorkspace().findSync(node.url).removeWorkingCopy();
							}
	...
	close: function(){
		...
		if (this.cssFiles) {
			this.cssFiles.forEach(function(file){
				file.close();
				require("qfacex/ui/ide/model/Factory").closeModel(file);  // return the model to the factory
			}.bind(this));
		}
