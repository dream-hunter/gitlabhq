...
], function(declare, Widget, ImageUtils, States,Metadata){

 var ModifyCommand =  declare(null, {
	name: "modify",
	...
	execute: function(){
		...
		this._workbench.publish("/davinci/ui/widget/replaced", newWidget, widget);
		...
		if (this._isRefreshParentOnPropChange(widget)) {
			...
			var command =
					new ModifyCommand(parentWidget,
							null, null, parentWidget._edit_context);
			command.execute();
		}
		
		this._workbench.publish("/davinci/ui/widgetPropertiesChanged", [newWidget]);
	},
	...
	_isRefreshParentOnPropChange: function(widget) {
		return Metadata.queryDescriptor(widget.type, "refreshParentOnPropChange");
	},
