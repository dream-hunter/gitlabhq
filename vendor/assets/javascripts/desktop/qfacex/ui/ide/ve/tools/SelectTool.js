...
return declare(tool, {
		...
		if(moverWidget){
			...
			if(userDojo){
				...
				if(!(helper && helper.disableDragging && helper.disableDragging(moverWidget)) &&
					...
					this._context._workbench.domNode.focus();
			...
			if(!absolute) {
				...
				if(ppw){
					...
					dojo.forEach(reorderedSelection, function(w){
						...
						compoundCommand.add(new AddCommand(newwidget, ppw.parent, idx,this._context));
				...
				if(copy){
					...
					dojo.forEach(reorderedSelection, function(w){
						...
						NewWidgets.push(newwidget);
						if(proposedParent){
							compoundCommand.add(new AddCommand(newwidget, proposedParent, -1 /*append*/,this._context));
						}else{
							compoundCommand.add(new AddCommand(newwidget, parentWidget, widx,this._context));
						}
						newselection.push(newwidget);
					}, this);
	...
	onMove: function(mover, box, event){
		...
		var editorPrefs = Preferences.getPreferences('davinci.ve.editorPrefs', 
				this._context._workbench.getProject());
		var doSnapLinesX = (!this._shiftKey && this._moverAbsolute); //(!this._shiftKey && editorPrefs.snap && this._moverAbsolute); modified by LWF
