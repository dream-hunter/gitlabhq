...
return declare( null, {
	...
	getHelper: function() {
        if (!this._edit_helper) {
            this._edit_helper = require("qfacex/ui/ide/ve/widget").getWidgetHelper(this.type);
        }
	...
	getParent: function() {
		return require("qfacex/ui/ide/ve/widget").getEnclosingWidget(this.domNode.parentNode) || this.domNode.parentNode;
	},
	...
	_styleText: function (v) {
		...
		if(dojo.isArray(v)){
			var vArray = States.normalizeArray("style", this.domNode, name, v);
			...
			for(var name in v){
				...
				value = States.normalize("style", this.domNode, name, v[name]);
				if(value !== undefined && value != "" && value!=null) {
					s += name + ": " + v[name] + "; ";
				}
			}
	...
	_getData: function(options) {
		var data = {type: this.type, properties: {}},
			widgetUtils = require("qfacex/ui/ide/ve/widget");
		...
		if(properties) {
			for(var name in properties) {
				...
				/*if(name == "theme") {
					value = require("qfacex/ui/ide/ve/widget").getPropertyValue(widget, name).themeName;
	...
	getStyleValues: function() {
		...
		var values = require("qfacex/ui/ide/ve/widget").parseStyleValues(text);
	...
	setStyleValuesAllStates: function(styleValuesAllStates){
		...
		if(styleValuesAllStates){
			for(var state in styleValuesAllStates){
				...
				States.setStyle(this.domNode, state, styleArray, true /*silent*/);
			}
		}
	},
