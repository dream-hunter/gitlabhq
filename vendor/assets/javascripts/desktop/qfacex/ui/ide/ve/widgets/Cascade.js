define( ["dojo/_base/declare",
        ...
        "qface/utils/markup/css/CSSModel",
        "qfacex/ui/ide/ui/widgets/DocileDialog",
        "qfacex/ui/ide/ve/States",
        "dojo/i18n!../nls/ve",
		"qfacex/ui/workbench/WorkbenchPart"
],function(declare,WidgetLite,Preferences, CSSModel, DocileDialog, States, veNLS, WorkbenchPart){
	var __id = 0;
	var Cascade =  declare( [WidgetLite,WorkbenchPart], {
		...
		constructor: function(params, srcNodeRef){
			...
			this._radioGroupName =  "qfacex_ui_ide_ve_widgets_Cascade" + (__id++);
			this._handles = [];
		...
		_onFieldChange : function(){
			...
			if(askUserResponse === false){
				...
				if(this._values[this._targetValueIndex].readOnly && this._editor.editorID != 'davinci.themeEdit.ThemeEditor'){ // #23 theme editor only writes out deltas
					...
					this.getWorkbench().showMessage(langObj.errorModifyingValue, content, {width: 350}, dojo.hitch(this, function(){
						innerResolveFunc();
						return true;
					}));
					if(cascadeBatch){
					...
					this._setFieldValue(this._value,this._loc);
						  ...
						  // editorPrefs.cssOverrideWarn && modified by LWF
							this._editor.supports("MultiPropTarget")){
					require(['qfacex/ui/ide/ve/widgets/ChangeWillModify'], dojo.hitch(this, function(ChangeWillModify) {
							function _submit() {
								if(cascadeBatch){
							...
							var dialog = this.getWorkbench().showDialog("", cwm, {width: 350}, dojo.hitch(this, _submit), null, null, true);
		...
		_changeValue : function(targetIndex,value){
			...
			if(targetRule.type=="element.style"){
				...
				this.publish("/davinci/ui/styleValuesChange",{values:valueObject, appliesTo:'inline', applyToWhichStates:applyToWhichStates });
			}else{
				this.publish("/davinci/ui/styleValuesChange",{values:valueObject, appliesTo:targetRule, applyToWhichStates:applyToWhichStates });
			}
		},
		...
		_addDeltaRules : function(widget, values){
			...
			if (dynamicThemeUrl){
				var file = this.getWorkbench().getUserWorkspace().findSync(dynamicThemeUrl);
