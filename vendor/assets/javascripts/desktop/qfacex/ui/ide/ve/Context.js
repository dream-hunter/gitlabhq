define( [
	...
	"dojo/_base/connect",
	"dojo/window",
    "qfacex/ui/ide/ve/ThemeModifier",
	"qfacex/ui/ide/commands/CommandStack",
	"qfacex/ui/ide/ve/tools/SelectTool",
	"qface/lang/Path",
	"qfacex/ui/ide/ve/widget",
	"qfacex/ui/ide/ve/Focus",
	"qfacex/ui/ide/ve/metadata",
	"qfacex/ui/ide/ve/ChooseParent",
	"qfacex/ui/ide/ve/Snap",
	"qfacex/ui/ide/ve/States",
	"qfacex/ui/ide/XPathUtils",
	"qface/utils/markup/html/HtmlFileXPathAdapter",
	"qfacex/ui/ide/ve/HTMLWidget",
	"qface/utils/markup/css/CSSModel", // shorthands
	"qface/utils/markup/css/CSSRule",
	"qface/utils/markup/css/CSSImport",
	"qface/utils/markup/html/HTMLElement",
	"qface/utils/markup/html/HTMLText",
	"qfacex/ui/ide/workbench/Preferences",
	"qfacex/ui/ide/preview/silhouetteiframe",
...
var _preferences = {}; //FIXME: belongs in another object with a proper dependency
...
=====*/
	...
	constructor: function(args) {
		...
		this._contentStyleSheet = require.toUrl("qfacex/ui/ide/ve/resources/content.css");//Workbench.location() + require.toUrl("qfacex/ui/ide/ve/resources/content.css"); modified by LWF
		this._id = "_edit_context_" + contextCount++;
		this.widgetHash = {};
		
		lang.mixin(this, args);

		this._workbench =  args.editor._workbench;
	...
	activate: function(){
		...
		this._workbench.publish("/davinci/ui/context/statesLoaded", this);
		this._onLoadHelpers();
	...
	getBaseResource: function(options){
		return this._workbench.getUserWorkspace().findSync(this.getDocumentLocation());
	},
	...
	_getWidgetFolder: function(){
		...
		if(!prefs.widgetFolder){
			...
			Preferences.savePreferences(this._workbench,'davinci.ui.ProjectPrefs',base, prefs);
		}
	...
	*/	
	...
	getDojoUrl: function(){
		var loc=this._workbench.location();
	...
	getBase: function(){
		if(this._workbench.singleProjectMode()) {
			return this._workbench.getProject();
		}
	},
	...
	getCurrentBasePath: function(){
		var base = new Path(this._workbench.getProject());
		var prefs = Preferences.getPreferences('davinci.ui.ProjectPrefs',base);
		if(prefs && prefs.webContentFolder!==null && prefs.webContentFolder!==""){
    ...
    loadTheme: function(newHtmlParms){
		...
		var allThemes = Library.getThemes(this._workbench.getProject()),
			themeHash = {},
			defaultTheme;
	...
	_setSource: function(source, callback, scope, newHtmlParams){
		...
		if (this.frameNode) {
			...
			*/
			...
			var realUrl = this._workbench.location() + "/" ;
...
*/
		...
		// Code is also injected into the page via workbench/qfacex/ui/ide/davinci.js to do this at runtime.
		...
		window.setTimeout(function(){
			...
			window.setTimeout(function(){
				this.widgetAddedOrDeleted();
				 this._workbench.publish("/davinci/ui/context/loaded", this);
		...
		all(prereqs).then(function() {
			this.getGlobal()["require"]("dojo/ready")(function(){
				try {
					...
					this._editorSelectConnection = this._workbench.subscribe("/davinci/ui/editorSelected",
							dojo.hitch(this,_editorSelectionChange));
...
//		Widget.setStyleValues(this.container, values); //TODO
	...
	_preserveStates: function(node, cache){
		var statesAttributes = States.retrieve(node);
...
//FIXME: Logic doesn't completely deal with nesting yet.
				...
				var oldValue = srcElement.getAttribute(States.APPSTATES_ATTRIBUTE);
				if(oldValue != maqAppStatesString){
					srcElement.setAttribute(States.APPSTATES_ATTRIBUTE, maqAppStatesString);
					visualChanged = true;
				}
				// Remove any lingering old dvStates attribute from model
				if(srcElement.hasAttribute(States.APPSTATES_ATTRIBUTE_P6)){
					srcElement.removeAttribute(States.APPSTATES_ATTRIBUTE_P6);
					visualChanged = true;
				}
			}
			if(maqDeltasString){
...
//FIXME: Logic doesn't completely deal with nesting yet.
				// Migrate states attribute names in the model
				var oldValue = srcElement.getAttribute(States.DELTAS_ATTRIBUTE);
				if(oldValue != maqDeltasString){
					srcElement.setAttribute(States.DELTAS_ATTRIBUTE, maqDeltasString);
					visualChanged = true;
				}
				// Remove any lingering old dvStates attribute from model
				if(srcElement.hasAttribute(States.DELTAS_ATTRIBUTE_P6)){
					srcElement.removeAttribute(States.DELTAS_ATTRIBUTE_P6);
					visualChanged = true;
				}
			}
			if(visualChanged){
			...
			States.store(widget.domNode, maqAppStates, maqDeltas);
...
*/
		...
		States.removeUnusedStates(this);
	...
	_AppStatesActivateActions: function(){
		if(this.editor.isInstanceOf(require("qfacex/ui/ide/ve/PageEditor"))){
			return;
		}
	...
	focus: function(state, index, inline){
		...
		if(index < this._focuses.length){
			...
			dojo.withDoc(this.getDocument(), dojo.hitch(this, function(){
				focus = new Focus({workbench:this._workbench});
	...
	getPreferences: function(){
		return lang.mixin({}, _preferences);
	},
	...
	getFlowLayout: function() {
		...
		if (!flowLayout){ // if flowLayout has not been set in the context check the edit prefs
			//var editorPrefs = Preferences.getPreferences('davinci.ve.editorPrefs', this._workbench.getProject());
	...
	setActiveTool: function(tool){
		...
		this._workbench.publish("/davinci/ve/activeToolChanged",this, tool);
	},
	 ...
	 * @param  {qfacex/ui/ide/html/HTMLFile} source
...
//FIXME: Need to generalize beyond just BODY
			var states = bodyElement.getAttribute(States.APPSTATES_ATTRIBUTE);
			if(!states){
				// Previous versions used different attribute name (ie, 'dvStates')
				states = bodyElement.getAttribute(States.APPSTATES_ATTRIBUTE_P6);
				if(states){
					bodyElement.setAttribute(States.APPSTATES_ATTRIBUTE, states);
				}
			}
			// Remove any lingering old dvStates attribute from model
			bodyElement.removeAttribute(States.APPSTATES_ATTRIBUTE_P6);
	...
	onContentChange: function(){
		...
		setTimeout(lang.hitch(this,function(){
			...
			this._workbench._autoSave();
		}), 0);
	},

	onSelectionChange: function(selection){
		this._cssCache = {};
		this._workbench.publish("/davinci/ui/widgetSelected",selection);
	},
...
*/
	...
	getStyleAttributeValues: function(widget){
		...
		if (!isNormalState) {
			...
			var stateStyleValuesArray = States.getStyle(widget.domNode, currentStatesList);
...
////////////////////////////////////////////////////////////////////////////////
	...
	registerSceneManager: function(sceneManager){
		...
		if(!this.sceneManagers[id]){
			...
			this._workbench.publish("/davinci/ui/context/registerSceneManager", sceneManager);
		}
	},
	...
	*/
	...
	getFocusContainer: function(){
		return this._workbench.focusContainer; //document.getElementById('focusContainer');
	},
