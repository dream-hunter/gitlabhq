define( [
    	...
    	"qfacex/ui/ide/ve/utils/ImageUtils",
...
return declare( null, {
	...
	constructor: function(widget, parent, index,context){
		if(widget){
	...
	execute: function(){
		...
		var parent = Widget.byId(this._parentId,this._context.getDocument(),this._context); 
		if(!parent){
			return;
		}
		...
		if(this._data){
			...
			widget = Widget.byId(this._id,this._context.getDocument(),this._context); //,context);
		}
		if(!widget){
			return;
		}
		...
		if(this._index && typeof this._index != "number") {
			if (this._index.domNode){ // widget
				...
				var w = Widget.byId(this._index.id,this._context.getDocument(),this._context); //,context);
	...
	undo: function(){
		...
		var widget = Widget.byId(this._id,this._context.getDocument(),this._context);
		if(!widget){
			return;
		}
