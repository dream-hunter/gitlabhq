require({cache:{
'url:qfacex/ui/ide/ve/template.html':"<!DOCTYPE html>\n<html>\n<head>\n<meta charset=\"utf-8\" />\n<title>Untitled</title>\n</head>\n<body>\n</body>\n</html>\n"}});
define( [
	"dojo/_base/declare",
	"dojo/_base/lang",
	"dojo/_base/connect",
...
var VisualEditor = declare(  null,  {
	...
	constructor: function(element, pageEditor)	{
		...
		this._workbench = pageEditor._workbench;
		...
		this._subscriptions.push(this._workbench.subscribe("/davinci/ui/editorSelected", this._editorSelected.bind(this)));
		this._subscriptions.push(this._workbench.subscribe("/davinci/ui/context/loaded", this._contextLoaded.bind(this)));
		
		var editorsStackContainer = this._workbench.editorsStackContainer.domNode;
		var visualEditorBorder =dojo.query(".visualEditorBorder",editorsStackContainer ); //document.getElementById('visualEditorBorder');
		if(!visualEditorBorder){
			visualEditorBorder = dojo.create('div', {className:'visualEditorBorder'}, editorsStackContainer);
			dojo.create('div', {className:'visualEditorBorderTopLeft'}, visualEditorBorder);
			dojo.create('div', {className:'visualEditorBorderTopRight'}, visualEditorBorder);
			dojo.create('div', {className:'visualEditorBorderTop'}, visualEditorBorder);
			dojo.create('div', {className:'visualEditorBorderRight'}, visualEditorBorder);
			dojo.create('div', {className:'visualEditorBorderLeft'}, visualEditorBorder);
		}
	...
	setDevice: function(deviceName) {
		...
		context.clearCachedWidgetBounds();
		this._workbench.publish("/davinci/ui/deviceChanged", deviceName);
		this._workbench.publish("/davinci/ui/repositionFocusContainer");
	...
	isActiveEditor: function(){
		var currentEditor = this._workbench.currentEditor;
		return currentEditor && currentEditor.isInstanceOf(require("qfacex/ui/ide/ve/PageEditor")) && currentEditor.visualEditor == this;
	},
	...
	_stylePropertiesChange: function (value){
		...
		if(command){
			...
			this._srcChanged();
			this._workbench.publish("/davinci/ui/widgetValuesChanged",value);
		}
	},
	...
	_setContentRaw: function(filename, content, newHtmlParams){
		...
		if (!this.initialSet){
		   	var workspaceUrl = this._workbench.getUserWorkspaceUrl();
			...
			var prefs=Preferences.getPreferences('davinci.ve.editorPrefs', this._workbench.getProject());
			if (prefs) {
...
//			this._handles.push(dojo.connect(this.context, "onSelectionChange",this, this.onContentChange));
	...
	_connectCallback: function(failureInfo) {
		try {
			...
			popup = this._workbench.createPopup({
			...
			this._focusPopup = this._workbench.createPopup({
				partID: 'davinci.ve.visualEditor',
			...
			this._workbench.publish('/qfacex/ui/ide/ui/context/pagebuilt', context);
		} catch(e) {
			failureInfo = e;
		} finally {
		...
		var visitor = {
				visit: function(node){
					if((node.elementType=="HTMLFile" || node.elementType=="CSSFile") && node.isDirty()){
						...
						workspace.findSync(url).removeWorkingCopy();
	...
	previewInBrowser: function(){
		var deviceName = this.deviceName,
			editor = thiw._workbench.getOpenEditor(),
		...
		if(deviceName && deviceName.length && deviceName != 'none'){
			...
			fileURL = thiw._workbench.location();
			if (this._orientation == 'landscape') {
	...
	_editorSelected: function(event){
		...
		if(visualEditorBorder){
			...
			if(!event.editor || event.editor.isInstanceOf(PageEditorClass)){
