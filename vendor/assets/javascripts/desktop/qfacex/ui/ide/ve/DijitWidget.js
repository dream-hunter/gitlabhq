...
return declare( _Widget, {
	...
	constructor: function(mixin, node, dijitWidget, metadata, srcElement) {
		...
		var allowedChild = Metadata.getAllowedChild(this.type);
	...
	getParent: function() {
		...
		do{
			widget = require("qfacex/ui/ide/ve/widget").getEnclosingWidget(this.dijitWidget.domNode.parentNode);
	...
	_getChildren: function(attach) {
		...
		if (Metadata.getAllowedChild(this.type)[0] !== 'NONE') {
			this.dijitWidget.getChildren().forEach(function(child) {
				if (attach) {
					children.push(require("qfacex/ui/ide/ve/widget").getWidget(child.domNode));
				} else {
