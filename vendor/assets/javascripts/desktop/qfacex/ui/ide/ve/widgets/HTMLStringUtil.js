define([
	"require",
	"qfacex/ui/tool/FontDataStore",
	"qfacex/ui/tool/FontComboBox",
	"qfacex/ui/tool/MultiInputDropDown",
	"dijit/form/ComboBox",
	"./MetaDataStore",
	"qfacex/ui/tool/ColorPicker",
	"qfacex/ui/tool/Background"
],function(require, FontDataStore){
	...
	dojo.mixin(HTMLStringUtil, {
		...
		getEditor: function(jsonString){
			...
			switch (metaType){
				case "multi":
					...
					var text = "<div data-dojo-type='qfacex/ui/tool/MultiInputDropDown' " + valuesText + "  class='propertyPaneEditablevalue' style='display:inline-block; width:100%;' id='"+ id + "'"+disabled+"></div>";
				...
				case "comboEdit":
					...
					var text = "<select data-dojo-type='dijit/form/ComboBox' autoComplete='false' style='display:inline-block; width:100%;' id='"+ id + "'"+disabled+">";
				...
				case "combo":
					...
					return text;
				case "font":
					var text = "<div data-dojo-type='qfacex/ui/tool/FontDataStore' jsId='"+ id + ('_fontStore') + "'>";
						text+= "<div data-dojo-type='qfacex/ui/tool/FontComboBox' value='" + FontDataStore.fonts[0].value + "' store='"+ id + ('_fontStore') + "'  id='"+ id +"' class='propertyPaneEditablevalue' style='display:inline-block; width:100%;'></div>";
					return text;
				case "state":
					var text="<div data-dojo-type='qfacex/ui/ide/ve/widgets/MetaDataStore' jsId='davinci.properties.event"+ (id) + ('_Store') + "'>";
					text+="<div data-dojo-type='dijit/form/ComboBox' id='"+ id +"'store='davinci.properties.event"+ (id) + ('_Store') + "' class='propertyPaneEditablevalue' style='display:inline-block; width:100%;' autoComplete='false'></div>";
					return text;
				case "color":
					var text = "<div class='propertyPaneEditablevalue' data-dojo-type='qfacex/ui/tool/ColorPicker' id='"+ id + "' ></div>";
					return text;
				...
				case "background":
					...
					var text="<div data-dojo-type='qfacex/ui/tool/Background' data-dojo-props='workbench:this._workbench'  id='" + id + "'" + valuesText + propNameText + swatchText + "></div>";
					return text;		
		...
		loadTemplate: function(templatePath, nameSpaceBase){
			...
			var text = dojo.xhrGet({url:url, handleAs:"text", sync:true  });
			return text;
		},
			...
			for(var i=0;i<page.length;i++){
					...
					//	page[i].showHelp = this.getId();
						...
						if(page[i].target && !page[i].hideCascade){
							...
							htmlText+= " onClick=\"require('qfacex/ui/ide/ve/widgets/HTMLStringUtil').showProperty(";
						...
						if(page[i].target && !page[i].hideCascade){
							...
							htmlText+="<td colspan='5' width='100%' class='showCascadeDiv'><div data-dojo-type='qfacex/ui/ide/ve/widgets/Cascade' data-dojo-props='workbench:this._workbench' toggleClasses=" + toggleClasses + " target='" + dojo.toJson(page[i].target)+"' targetField='\"" + page[i].id + "\"' id='" + page[i].cascadeSection + "'></div></td></tr>";
						}
					}else{
						htmlText+="</table>";
		...
		showProperty: function(propertyRowId,workbench){
			...
			//dojo.addClass(workbench.davinci_app.domNode,"showingCascade");//TODO 
			...
			var SwitchingStyleView = require("qfacex/ui/ide/ve/views/SwitchingStyleView");//dojo.getObject("davinci.ve.views.SwitchingStyleView");
			var sectionTitle = SwitchingStyleView.prototype.sectionTitleFromKey(sectionKey);
		
			//if(Runtime.supportsCSS3Transitions){ // modified by LWF
			var thisStyle = document.body.style;
		    var supportsCSS3Transitions = thisStyle.WebkitTransition !== undefined ||
