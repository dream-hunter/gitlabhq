define("qfacex/ui/ide/model/Factory", [
	"dojo/_base/declare",
	"qface/utils/markup/css/CSSFile",
	"qface/utils/script/js/JSFile",
	"qface/utils/markup/html/HTMLFile",
	"qface/services/ofs/FileSystem"
], function(declare, CSSFile, JSFile, HTMLFile,FileSystem) {
		...
		for(var i = 0; i<_resources.length; i++) {
			if (_resources[i].url == url) {
				...
				if (_instances[i] === 0) {
					...
					// editor.
					var resource = FileSystem.findSync(url);
...
var _connection = require(["dojo/_base/connect"], function(connect) {
	connect.subscribe("qfacex/ui/ide/model/closeModel", /*context*/ Factory, Factory.closeModel);
}); 
