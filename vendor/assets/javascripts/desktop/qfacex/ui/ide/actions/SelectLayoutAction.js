require({cache:{
'url:qfacex/ui/ide/ui/templates/SwitchLayout.html':"<div>\n  <select data-dojo-type=\"dijit/form/ComboBox\" data-dojo-attach-point=\"combobox\">\n  \t<option>${langObj.abs}</option>\n  \t<option>${langObj.flow}</option>\n  </select>\n</div>\n"}});
define( [
      "dojo/_base/declare",
      "dijit/_WidgetBase",
...
], function(declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Action, langObj, templateString){

var = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
...
return declare("davinci.actions.SelectLayoutAction", Action, {
	...
	isEnabled: function(selection){
		...
		if (editor && editor.isInstanceOf && PageEditorClass && editor.isInstanceOf(PageEditorClass)) {
			return true;
		} else {
			return false;
		}
	...
	showLayouts: function(){
		...
		var ui = new SelectLayoutActionContent({flowLayout: flowLayout});

		function _callback() {
			this._changeLayoutCommand(ui.getValue());
		}

		this._workbench.showDialog(langObj.newWidgetsShouldUse, ui, {width: 200}, dojo.hitch(this, _callback), langObj.select);
	}
});
});
