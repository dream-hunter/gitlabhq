define([
        ...
        "qfacex/ui/tool/ColorPickerFlat",
        "qfacex/ui/tool/ColorStore",
        "qfacex/ui/tool/MutableStore",
        "dijit/form/ComboBox",
        "qfacex/ui/tool/BackgroundDialog",
        "qfacex/util/URLRewrite",
        "qface/lang/Path",
        "dojo/i18n!./nls/ve",
        "dojo/i18n!dijit/nls/common",
        "qfacex/util/CssUtils",
        "qfacex/ui/tool/ColorPicker"
        

],function(declare, Deferred, WidgetLite,WorkbenchPart, ColorPickerFlat, ColorStore, MutableStore, ComboBox,BackgroundDialog, URLRewrite, Path, veNLS, commonNLS, CssUtils, ColorPicker){
	...
	return declare([WidgetLite,WorkbenchPart], {
		...
		constructor : function() {
			this._BackgroundWidgets = this._workbench._BackgroundWidgets;
			if (!this._BackgroundWidgets) {
				this._BackgroundWidgets = this._workbench._BackgroundWidgets = {};
			};
		},
		...
		buildRendering: function(){
			...
			var langObjVE = this.langObjVE = veNLS;
	 		...
	 		if(colorswatch){
				...
				this._statics = ["", ColorPicker.divider, langObjVE.colorPicker, langObjVE.removeValue];
				this._run = {};
				if(!this.data ){
					...
					this.data.push({value:ColorPicker.divider});
			...
			var _BackgroundWidgets = this._BackgroundWidgets;
			if(typeof this.propname == 'string' && this._comboBox){
				...
				_BackgroundWidgets[this.propname] = {
						propPaletteWidget: this,
						comboBox: this._comboBox
				};
			}
			//this.inherited(arguments);  // modified by LWF 20130426
		},
		...
		startup : function(){
			...
			dojo.connect(this._button,"onclick",this,function(){
				...
				var background = new BackgroundDialog({context:context,workbench:this._workbench});	
				var executor = dojo.hitch(this, function(background){
					...
					if(!background.cancel){
						var xref = this._BackgroundWidgets;
				...
				this._workbench.showModal(background, "Background", '', executor); //modified by LWF 20130428
			});
			this.connect(this._comboBox, 'onChange', dojo.hitch(this, function(event){
				...
				if(event == ColorPicker.divider || event == this.langObjVE.colorPicker){
		...
		_onChange : function(event){
			...
			if(this._colorswatch && typeof event == 'string'){
				...
				}else if (event == ColorPicker.divider){
		...
		_chooseColorValue: function() {
			...
			ColorPickerFlat.show(this._colorPickerFlat, initialValue, this, isLeftToRight);
		}
