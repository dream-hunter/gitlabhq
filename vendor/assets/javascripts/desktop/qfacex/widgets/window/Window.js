...
define([
	...
	"dojo/i18n!./nls/window"
	],function(declare,lang,connect,array,query,domStyle,domGeometry,domClass,domConstruct,topic,Deferred,on,dom,_TemplatedMixin,
		BorderContainer,Menu,MenuItem,CheckedMenuItem,MenuSeparator,easing,ResizeHandle,Enum,nlsWindow) {
		var _container = null;
      ...
      this.resize({
		...
		_maxfull: function(isFull)	{
			...
			if (isFull) {
				...
				if (this.fulled) {
					...
					if (this._oldMarginBottom !== undefined) {
						if (this._oldMarginBottom.length) {
							this._oldMarginBottom = this._oldMarginBottom[0];
						}
						query('.win-mc',this.domNode).style("marginBottom",this._oldMarginBottom+"px");
						delete this._oldMarginBottom;
					}
				}
			}
		...
		full: function()	{
			this._maxfull(true);
			topic.publish("/qfacex/widgets/window/Window/full",this,true);
		},

		unfull: function(){
			this.unmaximize();
			topic.publish("/qfacex/widgets/window/Window/full",this,false);
		},

		expand: function(){
			this._maxfull(true);
			query(".win-bmw",this.domNode).style("border","0");
			query(".win-bmw",this.domNode).style("bottom","0");
			query(".win-bl",this.domNode).style("display","none");
		},
