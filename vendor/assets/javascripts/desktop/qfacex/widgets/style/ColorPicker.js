define([
    ...
    "dijit/_WidgetBase",
    "dijit/form/ComboBox",
    "qfacex/widgets/tool/ColorStore",
    "qfacex/widgets/tool/ColorPickerFlat",
    "dijit/TooltipDialog",
    "dojo/i18n!./nls/color",
    "dijit/popup"
       
],function(dojo,_WidgetBase, ComboBox, ColorStore, ColorPickerFlat, TooltipDialog, colorNLS){
 var colorPicker = dojo.eclare("qface.ui.widget.ColorPicker", [_WidgetBase], {
		...
		postCreate: function(){
			...
			this.domNode.removeAttribute("data-dojo-type"); // backwards compat

			this._statics = ["", davinci.ve.widgets.ColorPicker.divider, colorNLS.colorPicker, colorNLS.removeValue];
			this._run = {};
			if(!this.data ){
				...
				this.data.push({value:colorNLS.removeValue,run:function(){this.set('value','');}});
			}
			var displayValues = [];
		...
		_chooseColorValue: function(){
			...
			var	dialog = new TooltipDialog({title: colorNLS.selectColor, content: content});
